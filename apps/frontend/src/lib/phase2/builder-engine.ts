if (typeof window !== 'undefined') {
  throw new Error('Security Error: This module can only be executed on the server.')
}
...
function replaceHeroContent(
  ...
  let html = heroHTML
  html = html.replace(/<h1([^>]*)>[\s\S]*?<\/h1>/i, `<h1$1>${headline}</h1>`)
  html = html.replace(/<p class="subtitle"([^>]*)>[\s\S]*?<\/p>|<h2([^>]*)>[\s\S]*?<\/h2>/i, (match, p1, p2) => {
    if (p1 !== undefined) return `<p class="subtitle"${p1}>${subheading}</p>`
    if (p2 !== undefined) return `<h2${p2}>${subheading}</h2>`
    return match
  })
  html = html.replace(/<button([^>]*)>[\s\S]*?<\/button>|<a([^>]*class="cta"[^>]*)>[\s\S]*?<\/a>/i, (match, p1, p2) => {
    if (p1 !== undefined) return `<button${p1}>${ctaText}</button>`
    if (p2 !== undefined) return `<a${p2}>${ctaText}</a>`
    return match
  })
  return html
}
...
function replaceFeatures(
  ...
  let headIdx = 0
  html = html.replace(/<h3([^>]*)>[\s\S]*?<\/h3>/gi, (match, attrs) => {
    const replacement = headlines[headIdx] !== undefined ? headlines[headIdx] : ''
    headIdx++
    return `<h3${attrs}>${replacement}</h3>`
  })

  let descIdx = 0
  html = html.replace(/<p([^>]*)>[\s\S]*?<\/p>/gi, (match, attrs) => {
    // Only replace descriptions for feature blocks
    if (attrs.includes('description') || attrs.includes('text') || descIdx < descriptions.length) {
      const replacement = descriptions[descIdx] !== undefined ? descriptions[descIdx] : ''
      descIdx++
  ...
  tiers.forEach((tier) => {
    tiersHTML += `
      <div class="pricing-card">
        ...
        <ul>
          ${tier.features.map((f: any) => `<li>${f}</li>`).join('')}
        </ul>
...
function replaceFooterContent(
  ...
  let html = footerHTML
  if (company) html = html.replace(/<p class="company"([^>]*)>[\s\S]*?<\/p>/i, `<p class="company"$1>${company}</p>`)
  if (email) html = html.replace(/[\w.-]+@[\w.-]+\.\w+/i, email)
  if (phone) html = html.replace(/\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}/i, phone)
  return html
}
